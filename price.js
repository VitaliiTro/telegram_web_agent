    ...
    function loadPromoPricesFromFile(filePath) {
        try {
            ...
            tableRows.forEach(row => {
                ...
                const promoPriceCell = row.querySelector("td:nth-child(4)"); // Четверта колонка для промо-ціни

                if (itemNameCell && promoPriceCell) {
                    const itemName = itemNameCell.textContent.trim();
                    if (promoPrices[itemName]) {
                        promoPriceCell.textContent = parseFloat(promoPrices[itemName]).toFixed(2); // Форматування ціни
                    }
                }
            });
